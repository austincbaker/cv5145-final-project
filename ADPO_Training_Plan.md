...
| `train_model/data/generated_questions.json` | 14,195 multi-choice questions across 2,686 videos, produced by `prompt_generator/` plus the normal-video negatives emitted by `train_model/sft/add_normal_questions.py` | not in git |

Of the 2,972 raw clips, 2,687 have annotations and **2,686** of those are
referenced in `generated_questions.json`. The single unused annotation is
the malformed record that was left in the file but has no clip counterpart;
every annotated clip that exists on disk is now in the pipeline.

Question composition in `generated_questions.json`:

* **11,943** primary questions (standard aggression recognition).
* **2,252** trick / negative questions (`is_trick=true`). These split into:
  * **1,286** original trick questions — apparent-aggression-but-actually-not
    footage, correct answer is a "none" option.
  * **966** synthesised negatives — 14 question types × 69 normal
    (`normal_ucf_crime_*.mp4`) clips where nothing aggressive happens at
    all; correct answer is the "none" option, distractors are real
    aggression descriptions sampled from other videos. Generated by
    `train_model/sft/add_normal_questions.py` so the SFT objective
    includes examples of "pick 'no aggression'" on genuinely empty
    footage, not just on borderline cases.
* **0** secondary questions in the current build.
* **13,295** questions have 8 options (1 correct + 7 distractors).
* **900** questions (`role_identification`) have 4 options.
* 15 distinct `question_type` values in all.

Every `is_trick=True` question is included in the SFT splits but skipped
during Phase 4 DPO pair extraction (contrastive preferences over prose
aggression descriptions vs. a "none" answer aren't structurally
comparable).
...
Current cache: **2,686 videos × 8 frames = 21,488 JPEGs**. After
removing 7 unusable annotations (1 blank `file_name` + 6 missing
UCF-Crime clips) and adding the 69 `normal_ucf_crime_*.mp4` negatives
to `generated_questions.json`, Phase 0 reports zero failures.
