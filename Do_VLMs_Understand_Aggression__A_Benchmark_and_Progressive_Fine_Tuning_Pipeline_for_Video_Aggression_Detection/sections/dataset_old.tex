\section{Benchmarking Aggresive-Action Association}

\subsection{Video Corpus}

Our evaluation corpus comprises 2,688 video clips depicting interpersonal interactions, of which 2,239 contain aggressive behavior and 449 are non-aggressive controls.
...
The inclusion of videos from diverse sources ensures variation in recording quality, camera angle, resolution, and scene composition, reflecting the heterogeneity of real-world security and bystander footage.

\subsection{Annotation Schema}

Each video was manually annotated with structured metadata describing the aggressive interaction.
Annotations include: (1) the aggressive action being performed, with a total of 20 action categories and a control "none' category where no aggressive actions are taking place; (2) a natural-language description of the aggressor; (3) a natural-language description of the victim; (4) the environment or setting; and (5) descriptions of any bystanders present but not directly participating.
...
Bystanders are present in 65.3\% of aggressive videos ($n = 1,462$).
On average, each aggressive video contains 2.79 annotated individuals across all roles.

\subsection{Benchmark Question Generation - Should this be here or in methods???}
...
By combining attributes, these questions exponentially expand the answer space since correctly identifying one component but not the other still yields an incorrect response.
...
\begin{enumerate}
  ...
  \item \textbf{Role reversals.} In compound and sequence questions, the aggressor and victim are swapped to produce a distractor that tests whether the model understands directionality rather than merely detecting the presence of specific individuals.
