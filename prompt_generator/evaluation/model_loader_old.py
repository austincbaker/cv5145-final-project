...
class ModelConfig:
    ...
    thinking_budget: int = 128   # Conservative default to prevent token overflow
    max_new_tokens: int = 64     # Answers are just numbers, don't need many tokens
...
class OvisModelLoader:
    ...
    def generate_response(
        ...
        Args:
            ...
            prompt: Text prompt
            override_max_tokens: Override max_new_tokens (for token limit recovery)
            override_thinking_budget: Override thinking_budget (for token limit recovery)
        """
        ...
        max_new_tokens = override_max_tokens if override_max_tokens is not None else self.config.max_new_tokens
        thinking_budget = override_thinking_budget if override_thinking_budget is not None else self.config.thinking_budget
        
        # If using very limited tokens, disable thinking entirely
        enable_thinking = self.config.enable_thinking
        enable_thinking_budget = self.config.enable_thinking_budget
        if override_max_tokens is not None and override_max_tokens < 64:
