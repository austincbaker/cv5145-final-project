...
for p in $(seq 1 $EVAL_PARTS); do
    EVAL_JOB=$(sbatch --parsable --dependency=afterok:$ADPO_JOB \
        --job-name="5pct_av1_eval_p${p}" \
        --output="train_model/experiments/5pct_adaptive_v1/logs/eval_p${p}_%j.out" \
        --error="train_model/experiments/5pct_adaptive_v1/logs/eval_p${p}_%j.err" \
