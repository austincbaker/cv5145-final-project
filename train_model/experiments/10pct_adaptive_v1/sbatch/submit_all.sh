...
#
# Usage:
#   bash train_model/experiments/10pct_adaptive_v1/sbatch/submit_all.sh

set -e
cd "$(dirname "$0")/../../../.."

DIR="train_model/experiments/10pct_adaptive_v1/sbatch"
EVAL_PARTS=4
mkdir -p train_model/experiments/10pct_adaptive_v1/logs
...
for p in $(seq 1 $EVAL_PARTS); do
    EVAL_JOB=$(sbatch --parsable --dependency=afterok:$ADPO_JOB \
        --job-name="10pct_av1_eval_p${p}" \
        --output="train_model/experiments/10pct_adaptive_v1/logs/eval_p${p}_%j.out" \
        --error="train_model/experiments/10pct_adaptive_v1/logs/eval_p${p}_%j.err" \
